...
test $(find . -type f -name '*.go' -not -path "*.pb.go" -not -path "./.git/*" -not -path "./.workspace/*" | xargs ${GOPATH}/bin/gocyclo -over 17 2>&1 | wc -l) -gt 0 && echo "gocyclo   failed" && exit 1 || echo "gocyclo   succeeded"
